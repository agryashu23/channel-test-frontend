    ...
    builder
      ...
      .addCase(createChannel.fulfilled, (state, action) => {
        ...
        if (response.success && !response.limitReached) {
          ...
          state.userChannels.unshift(response.channel);
        }
      })
      ...
      .addCase(joinTopicInvite.fulfilled, (state, action) => {
        ...
        const response = action.payload;
        if (response.success && response.topicMembership && response.joined) {
          let index = state.userChannels.findIndex(
            (m) => m?._id === response.channel._id
          );
          if (index !== -1) {
            const topicIndex = state.userChannels[index].topics.findIndex(
              (topic) =>
                topic?._id.toString() === response.topic?._id.toString()
            );
            if (topicIndex === -1) {
              state.userChannels[index].topics.push(response.topic);
            }
      ...
      .addCase(joinTopic.fulfilled, (state, action) => {
        ...
        if (
          ...
          const index = state.userChannels.findIndex(
            (channel) => channel?._id === response.topic?.channel?._id
          );
          if (index !== -1) {
            const topicIndex = state.userChannels[index].topics.findIndex(
              (topic) =>
