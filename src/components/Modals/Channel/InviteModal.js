            ...
            {codeGenerated ?(
              ...
              <div className="flex relative items-center mt-2 bgBackground rounded-md pr-4 pb-3 w-full">
                ...
                <div className="flex flex-col gap-4 mb-6 pr-4">
                    <div>
                        <label className="block text-theme-secondaryText text-sm mb-1">Usage Limit of users</label>
                        <input
                        type="number"
                        min="1"
                    ...
                    >
                        {
                        expiryOptions.find((opt) => opt.value === expiryOption)?.label || "Select"
                        }
                        <svg
                    ...
                    {expiryOption === "custom" && (
                        ...
                        <label className="block text-theme-secondaryText text-sm mb-1">Custom Expiry Date & Time</label>
                        <DatePicker
                        selected={customExpiry ? new Date(customExpiry) : null}
                        onChange={(date) => setCustomExpiry(date?.toISOString() || "")}
                        showTimeSelect
                        timeFormat="HH:mm"
                        timeIntervals={60}
                    ...
                    {error && <p className="text-theme-error text-sm font-light mt-1">{error}</p>}
