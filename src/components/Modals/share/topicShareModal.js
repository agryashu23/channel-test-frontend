...
const TopicShareModal = () => {
  ...
  if (code) {
    ...
    shareUrl = `https://${domainUrl}/account/${username}/channel/${channelId}/c-id/topic/${topicId}`;
  }
          ...
          >
            ...
            <div className="flex relative items-center mt-2 bgBackground rounded-md pr-4 pb-3 w-full">
              ...
              <CopyToClipboard text={shareUrl} onCopy={handleCopy}>
                <button className="absolute right-6 bg-theme-secondaryText text-theme-primaryBackground font-normal px-2 text-xs xs:text-sm py-1  rounded-md">
                  {copied ? "Copied" : "Copy"}
                </button>
              </CopyToClipboard>
            </div>
