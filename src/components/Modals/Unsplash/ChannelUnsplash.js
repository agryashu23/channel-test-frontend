...
const ChannelUnsplash = () => {
  const dispatch = useDispatch();

  const handleClose = () => {
    dispatch(closeModal("modalChannelUnsplashOpen"));
    setSelectedImage(null);
    setImages([]);
    setSearchQuery("");
  };
  const isOpen = useSelector((state) => state.modals.modalChannelUnsplashOpen);
...
export default ChannelUnsplash;
