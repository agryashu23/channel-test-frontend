...
import { fetchBusinessChannelsTopics, fetchBusinessRolesMembers,clearMembers,updateUserBusinessRole,removeUserBusinessMember } from '../../../redux/slices/businessSlice';
...
const AdminRoleMembers = () => {
  ...
  const [fetchedContext, setFetchedContext] = useState({ channelId: null, topicId: null });
  ...
  const topicRoleOptions=["member","editor","admin"];
  ...
  const handleFetchMembers = async () => {
    ...
    if (res) {
      ...
      setFetchedContext({ channelId: selectedChannelId, topicId: selectedTopicId }); // ✅ update context
    }
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if(openRoleDropdownRef.current && !openRoleDropdownRef.current.contains(event.target)){
  ...
  const handleRoleChange = (userId, role) => {
    ...
    dispatch(updateUserBusinessRole(data)).unwrap().then((res)=>{
      if(res){
  ...
  return (
    <div className="flex flex-col pt-6  px-4 bg-theme-tertiaryBackground  w-full">
        ...
        <p className="text-theme-emptyEvent text-xs mb-3 italic font-light">• Selecting a channel fetches all its members. Selecting a topic 
          within that channel further filters to members of that specific topic. Removing members or updating their roles works contextually 
        ...
        <div className="flex flex-row flex-wrap gap-6 mb-4">
            <div className="flex flex-col w-full sm:w-auto relative" ref={channelDropdownRef}>
              <p className="text-sm text-theme-secondaryText mb-2">Fetch by Channel</p>
              <div
            ...
            {selectedChannelId && (
                <div className="flex flex-col w-full sm:w-auto relative" ref={topicDropdownRef}>
                  <p className="text-sm text-theme-secondaryText mb-2">Fetch by Topic</p>
                  <div
                    className="border border-theme-chatDivider rounded-md py-2 sm:px-3 px-2 text-sm 
                    bg-theme-tertiaryBackground cursor-pointer flex justify-between items-center text-theme-primaryText w-full sm:w-64"
                  ...
                  >
                    <span>
                      {availableTopics.find((t) => t._id === selectedTopicId)?.name || "Select Topic"}
                    </span>
                    <img
                      src={
                        topicDropdownOpen
          ...
          <div className="bg-theme-buttonEnable text-theme-secondaryText px-8 mt-1 py-1.5 w-max  font-normal  rounded-lg cursor-pointer"
          ...
          {business.loading ? (
            ...
            <div className="rounded-lg  flex flex-col sm:px-2 ">
              {filteredMembers.map((member) => (
                ...
                <div key={member._id} className="flex flex-row my-2 xs:items-center ">
                  <div className="flex xs:flex-row flex-col justify-between xs:items-center w-full">
                    <div className="flex flex-row items-center xs:mb-0 mb-2">
                      {member.user?.logo ? (
                        ...
                        <p className="text-theme-secondaryText font-normal sm:text-md text-sm truncate w-full cursor-pointer ml-2" 
                        onClick={()=>navigate(`/account/${member.user?.username}/profile`)}>
                    ...
                    >
                      <span>
                        {(selectedRoleMap[member._id] || member.role).charAt(0).toUpperCase() +
                          (selectedRoleMap[member._id] || member.role).slice(1)}
                      </span>
                      <img
                        src={openRoleDropdown === member._id ? ChannelImages.ArrowUp.default : ChannelImages.ArrowDown.default}
                        alt="arrow"
                        className="w-4 h-4 ml-2"
                      />
                    </div>
                    {openRoleDropdown === member._id && (
                      <div className="absolute z-10 mt-1 w-24 bg-theme-secondaryBackground border border-theme-chatDivider
                       rounded-md shadow-md max-h-60 overflow-y-auto" ref={openRoleDropdownRef}>
                        {(fetchedContext.topicId ? topicRoleOptions : channelRoleOptions)
                        .filter(role => role !== (selectedRoleMap[member._id] || member.role))
                        .map((role) => (
                          <div
                            key={role}
                            className="px-3 py-2 text-sm text-theme-primaryText cursor-pointer hover:bg-theme-primaryBackground"
                            onClick={(e) => {
                              e.stopPropagation();
                              setSelectedRoleMap((prev) => ({ ...prev, [member._id]: role }));
                              setOpenRoleDropdown(null);
                            }}
                          >
                  ...
                  <img
                    ...
                    />
                        {(selectedRoleMap[member._id] && selectedRoleMap[member._id] !== member.role) && (
                          <div
                            className="text-xs text-theme-secondaryText bg-theme-buttonEnable rounded-md px-2 py-1 font-light cursor-pointer"
                            onClick={() => handleRoleChange(member.user._id, selectedRoleMap[member._id])}
                          >
