...
import { setBusinessField, setBusinessParameterField, updateBusinessCustomizations } from '../../../redux/slices/businessSlice';
...
const AdminCustomizations = () => {
  ...
  const handleToggle = (key) => {
    if (key === 'allowDM' || key === 'talkToBrand') {
  ...
  const handleSave = async () => {
    try {
      const data = {
        ...
        whatsappNotifications: business.business?.whatsappNotifications ?? false,
  ...
  keyName === 'allowDM'
    ...
    return (
      ...
      <div className="flex items-center py-3">
        <span className="text-theme-secondaryText mr-4 sm:mr-6 sm:text-md text-sm">{label}</span>
        <button
  ...
  return (
    <div className="flex flex-col pt-6 sm:px-8 px-4 bg-theme-tertiaryBackground w-full min-h-screen">
      ...
      <p className="text-theme-secondaryText text-lg mt-1 font-normal">Customisations</p>
      <div className="border-t border-t-theme-chatDivider my-4"></div>

      <div className="space-y-5">
        <ToggleItem label="Whatsapp notifications" keyName="whatsappNotifications"  description="• Enable this to allow customizations for whatsapp notifications in topics."/>
        <ToggleItem label="Allow members to send direct message to you" keyName="talkToBrand" description="• Enable this to allow members to send direct message to you using Talk to us in topic page."/>
        <ToggleItem label="Allow members to send direct message to other members" keyName="allowDM" description="• Enable this to allow members to send direct message to other members in chats (DM)."/>
        <ToggleItem label="Chat summary generation" keyName="chatSummary" description="• Enable this to allow customizations for chat summary generation in topics."/>
        <ToggleItem label="Sync users permissions from your side" keyName="loginControl" description="• Syncing users permissions from your side will allow you to manage users permissions from your side. Like which channel/topic is accessible by which user. For more info contact support." />
      </div>
