    ...
    <div className="flex flex-col w-full h-full pt-2 overflow-y-auto">
      <div className="flex flex-row items-center justify-between px-4 pt-2 pb-3">
        <div className="flex flex-row items-center">
          <img src={Pin} alt="pin" className="w-5 h-5  dark:hidden block cursor-pointer"/>
          <img src={PinLight} alt="pin" className="w-5 h-5  dark:block hidden cursor-pointer"/>
          <p className="text-theme-secondaryText font-normal sm:text-[16px] text-sm ml-2">Pinned messages</p>
        </div>
        <div className="flex flex-row items-center w-max cursor-pointer " onClick={() => setIsPinned(false)}>
          <img src={Close} alt="close" className="w-3 h-3"/>
          <p className="text-theme-emptyEvent font-light text-sm ml-1">Close</p>
        </div>
      </div>
      {pinnedLoading ? <div className="flex flex-row items-center justify-center h-full">
        <Loading text="Loading pinned messages..."/>
      </div> : pinnedChats.length > 0 ? pinnedChats.map((chat) => (
        <div key={chat._id} className="border border-theme-chatDivider rounded-lg p-2 sm:mx-4 mx-3 cursor-pointer my-2" onClick={() => handleJumpToChat(chat._id)}>
          <div
                  ...
                  >
                    <div className="flex flex-row items-center justify-between w-full">
                      ...
                      >
                        ...
                        >
                          {chat.user?._id === myData?._id ? "You" : chat.user?.username}
                        </span>
                        <span className="font-light ml-1 text-xs">
                          {formatChatDate(chat.createdAt)}
                        </span>
                      </p>
                      <div className="bg-theme-secondaryText text-theme-primaryBackground 
        ...
        <p className="text-theme-emptyEvent font-light text-md">No pinned messages</p>
      </div>}
    </div>
  )
}
