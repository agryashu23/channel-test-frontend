...
const PageChatData = forwardRef(
  (
    ...
    useEffect(() => {
      ...
      dispatch(fetchTopicPollResponses(topicId));
    ...
    useEffect(() => {
      const handleReceiveMessage = (message) => {
        if (message?.topic === topicId && message?.user?.username) {
          if (
            myData?.username &&
            message?.user?.username !== myData?.username
          ) {
    ...
    const handlePinChat = (id, pinned) => {
      dispatch(pinChat(id))
        ...
        .then(() => {
          showCustomToast(
            `Chat ${pinned ? "unpinned" : "pinned"} successfully`
          );
        })
        .catch((error) => {
          console.log(error);
    ...
    const isAdmin = topic?.members?.find(
      (member) =>
        member?.user?.toString() === myUserId.toString() &&
    ...
    return (
      <>
        {isMemberClick ? (
          <div className="bg-theme-secondaryBackground w-full h-full overflow-y-auto">
            <TopicMembersTab
              isAdmin={isAdmin}
              topicId={topicId}
              topic={topic}
                  ...
                  >
                   ...
                   shadow-lg rounded-lg z-10`}
                            >
                              ...
                              {(chat.user?._id === myData?._id || isAdmin) && (
                                <div
                                  className="py-1 hover:bg-theme-primaryBackground"
                                  role="menu"
                                  aria-orientation="vertical"
                                  ...
                                  {media.type === "image" ? (
                                    <div className="relative h-36 mr-3">
                                      ...
                                      <button
                                        ...
                                        onClick={() => {
                                          const images = chat.media.filter(
                                            (c) => c.type === "image"
                                          );
                    ...
                    <div className={`flex flex-row space-x-2 mb-2 ${"ml-14"}`}>
                      {chat.reactions?.map((reaction, index) => (
                        <div
                          ...
                          onClick={() =>
                            handleReactionClick(reaction.type, chat._id)
                          }
                        >
                          {reaction.type === "❤️" ? (
                            <svg
                        ...
                        {reactionsData.map((reaction, index) =>
                          index === reactionsData.length - 1 ? (
                            <div
                              ...
                              key={`${reaction}-${reaction.createdAt}-1-${index}`}
                            >
                              <div
                                onClick={() =>
