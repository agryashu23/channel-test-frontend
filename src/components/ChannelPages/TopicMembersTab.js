...
const TopicMembersTab = ({ channelId, isOwner,topicId }) => {
  const dispatch = useDispatch();
  const { handleOpenModal } = useModal();
  const reorderMembers = useSelector((state) => state.reorderTopic.topicMembers);
  ...
  const handleRemoveMember = (user, topicId) => {
    dispatch(
      setReorderTopicField({ field: "removeTopicId", value: topicId })
    );
